...
class TransformerClassifier(nn.Module):
    def __init__(self, d_model, num_heads, user_keys, mood_keys, Genre_keys, num_hidden_layers, dropout=0.1):
        ...
        self.mother_tongue_pre_embed = nn.Embedding(num_embeddings=38, embedding_dim=1)
        ...
        self.W_zd = nn.Linear(d_model, 1, bias=False)
        self.W_z0 = nn.Linear(self.all_keys_number, 1, bias=False)
        self.W_zatt = nn.Linear(d_model, 1, bias=False)
...
mood_keys = ['mood', 'amazement', 'solemnity', 'tenderness', 'nostalgia', 'calmness', 'power', 'joyful_activation', 'tension', 'sadness']
...
def load_model():
    ...
    model.load_state_dict(torch.load('static/model_parameters.pt', map_location=torch.device('cpu')))
    model.eval()
    return model